---
source_file: "supabase/migrations/20260603_initial_schema.sql"
type: "code"
community: "Community None"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_None
---

# 20260603_initial_schema.sql

## Connections
- [[MENU_ITEMS]] - `contains` [EXTRACTED]
- [[menu_items]] - `contains` [EXTRACTED]
- [[order_items]] - `contains` [EXTRACTED]
- [[order_items_1]] - `contains` [EXTRACTED]
- [[orders]] - `contains` [EXTRACTED]
- [[orders_2]] - `contains` [EXTRACTED]
- [[profiles]] - `contains` [EXTRACTED]
- [[set_updated_at()]] - `contains` [EXTRACTED]
- [[trg_menu_items_updated_at]] - `contains` [EXTRACTED]
- [[trg_orders_updated_at]] - `contains` [EXTRACTED]
- [[trg_profiles_updated_at]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_None